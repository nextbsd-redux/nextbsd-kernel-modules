/*
 * Vendored from drm-kmod 6.12-lts (drivers/gpu/drm/drm_fbdev_shmem.c), which
 * ships the file but builds it into nothing, with four blocks wrapped in
 * #ifdef __linux__ -- exactly as drm-kmod itself already does in the sibling
 * drm_fbdev_ttm.c (see that file's "deferred I/O" block).
 *
 * WHY NO DEFERRED I/O IS NEEDED. Everything in Linux's fb_defio.c exists to
 * service a userspace mmap() of /dev/fbN, and drm-kmod creates no such cdev.
 * The console damage path is separate and already implemented and exported:
 *
 *   vt_drmfb -> info->fbops->fb_imageblit (FB_GEN_DEFAULT_DEFERRED_SYSMEM_OPS)
 *            -> drm_fb_helper_damage_area() -> drm_fb_helper_damage()
 *            -> fb_dirty -> drm_fbdev_shmem_helper_fb_dirty()
 *            -> fb->funcs->dirty == drm_atomic_helper_dirtyfb
 *            -> virtio_gpu_cmd_transfer_to_host_2d() + resource_flush
 *
 * vt(4) already batches at VT_TIMERFREQ (25 Hz), the same order as Linux's
 * HZ/20 defio delay, and drm_fb_helper coalesces damage into one clip rect,
 * so a periodic-flush shim would be strictly worse than what exists.
 *
 * Keep the diff to guards only, so the file stays rebaseable against drm-kmod.
 */
...
#else
	/*
	 * FreeBSD: fb_deferred_io_mmap() is a panic() stub in linux_fb.c, and
	 * vt(4) maps the console through vd_fb_mmap (vt_fb_mmap), never through
	 * fb_ops. Nothing legitimately reaches this path; refuse rather than
	 * hand out pages nothing would ever flush to the host.
	 */
	return (-ENODEV);
#endif
}
