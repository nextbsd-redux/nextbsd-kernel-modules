...
#ifndef __FreeBSD__
/*
 * EXCLUDED on FreeBSD: drm-kmod DOES compile this one function -- its
 * 15-line include/drm/drm_gem_atomic_helper.h stub declares exactly
 * drm_gem_plane_helper_prepare_fb() and nothing else, and drm.ko exports it.
 * Compiling it here too would be a duplicate symbol at load time.
 *
 * Everything BELOW this function -- the entire shadow-buffered plane API --
 * is what drm-kmod omits: no drm_shadow_plane_state, no
 * DRM_GEM_SHADOW_PLANE_FUNCS, no begin/end_shadow_fb_access. That is the
 * layer vboxvideo's cursor plane sits on, and the reason this file is
 * vendored at all.
 */
