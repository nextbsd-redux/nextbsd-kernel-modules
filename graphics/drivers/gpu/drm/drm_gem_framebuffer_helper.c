...
/*
 * Vendored from Linux v6.12, with FreeBSD deltas.
 *
 * drm-kmod (6.12-lts) ships a 76-line "Public domain" stand-in for this file
 * that implements only drm_gem_fb_get_obj, drm_gem_fb_destroy and
 * drm_gem_fb_create_handle -- while its include/drm/drm_gem_framebuffer_helper.h
 * still DECLARES the rest. So a driver using drm_gem_fb_create compiles clean
 * and then dies at load with
 *
 *     link_elf_obj: symbol drm_gem_fb_create undefined
 *
 * which kextload reports as "Exec format error". i915, amdgpu and radeon never
 * hit it because each implements its own .fb_create; bochs uses the generic
 * helper, so it is the first to need the real thing.
 *
 * The three functions drm-kmod's stub already defines and exports are excluded
 * under #ifndef __FreeBSD__ -- compiling our own copies would put duplicate
 * globals in the kernel symbol space. Everything else, notably
 * drm_gem_fb_create and the drm_gem_fb_funcs it hangs off, comes from here.
 */
/*
...
#endif /* !__FreeBSD__ -- drm-kmod's stub provides this */
