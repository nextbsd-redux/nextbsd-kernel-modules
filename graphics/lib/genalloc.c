...
#ifndef __FreeBSD__
/*
 * of_platform.h has no linuxkpi equivalent. It is needed only by
 * of_gen_pool_get(), the last function in this file, whose whole body sits
 * behind #ifdef CONFIG_OF -- and this build never defines CONFIG_OF (there is
 * no device tree on the PCI-attached GPUs this module serves). of.h and
 * platform_device.h DO exist in linuxkpi, so only this one include is guarded.
 */
